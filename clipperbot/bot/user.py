...
Assume the stream started at the hour mark.""")
    ...
    async def on_message_edit(self, before:discord.Message, after:discord.Message):
        ...
        if command_edit:
            ...
            # if sent_clip.relative_start is not None: # with plain c
                # not enough arguments
                if len(args) < 2:
                    return

                new_rel_start = -to_timedelta(args[1])
                old_args = sent_clip.command_ctx.message.content.split()
                if len(old_args) >= 3:
                    old_rel_start = to_timedelta(old_args[2])
                else:
                ...
                if len(args) < 3:
                    return
                # if the command is still there
                if args[0] not in {"c", "a"} and args[1] in {"fromstart", "s"}:
                    return                
        ...
        # replace the links.
            new_clip = await self._create_n_send_clip(sent_clip.command_ctx, new_ftime,
                new_duration, audio_only=sent_clip.audio_only,
                relative_start=None)
    ...
    async def _create_n_send_clip(self, ctx, from_time:dt.timedelta, 
        ...
        try:
           ...
           return await self._send_clip(ctx, from_time, duration, clip_fpath,
    ...
    async def _send_clip(self, ctx, from_time, duration, clip_fpath,
        ...
        if msg is not None:
            ...
            sent_clip = Clip(msg, clip_fpath, stream,
                from_time, duration, audio_only, ctx,
                relative_start=relative_start)
